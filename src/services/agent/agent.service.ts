...
export class AgentService implements IAgentService {
  ...
  async processOnboardStep1(
    ...
    if (!/^[0-9a-f]{64}$/i.test(input.publicKeyHex)) {
      throw Object.assign(new Error('Invalid public key'), { code: 'INVALID_PUBLIC_KEY', httpStatus: 400 });
    }
...
export function mapAgentError(error: unknown): { statusCode: number; code: string; message: string } {
