...
import { ErrorCode, buildDIDDocument, createStatusList, derivePublicKey } from '../../../src/core/index.js';
...
describe('VCService Branch Coverage', () => {
  ...
  describe('findActiveBySubjectDid', () => {
    ...
    it('returns the only active VC and parses JSON strings', async () => {
      ...
      await expect(
        service.findActiveBySubjectDid('did:1', 'HelixAgentCredential'),
    ...
    it('findActiveByVcIdForSubject returns a matching active VC only', async () => {
      ...
      await expect(
        service.findActiveByVcIdForSubject('vc-1', 'did:1', 'HelixAgentCredential'),
      ).resolves.toEqual({ id: 'vc-1', type: ['VerifiableCredential', 'HelixAgentCredential'] });
      await expect(
        service.findActiveByVcIdForSubject('vc-1', 'did:other', 'HelixAgentCredential'),
      ).resolves.toBeNull();
      await expect(
        service.findActiveByVcIdForSubject('vc-1', 'did:1', 'HelixUserCredential'),
      ).resolves.toBeNull();
    });
  });

  describe('record lookup branches', () => {
  ...
  describe('issueVC branches', () => {
    it('throws STATUS_LIST_INDEX_EXHAUSTED if list full', async () => {
      ...
      await expect(
        service.issueVC({ subjectDid: 'did:1', subjectType: 'user', userId: 'u1' }, 'req-1'),
    ...
    it('throws VALIDATION_ERROR for user with scopes', async () => {
      await expect(
        service.issueVC(
          { subjectDid: 'did:1', subjectType: 'user', userId: 'u1', privilegeScopes: ['read'] },
          'req-1',
        ),
    ...
    it('throws VALIDATION_ERROR for agent without scopes or name', async () => {
      await expect(
        service.issueVC({ subjectDid: 'did:1', subjectType: 'agent', agentName: '' }, 'req-1'),
      ).rejects.toMatchObject({ code: ErrorCode.VALIDATION_ERROR });
    });

    it('accepts any well-formed privilege scope', async () => {
  ...
  describe('revokeVC branches', () => {
    it('throws VC_NOT_FOUND if record missing', async () => {
      ...
      await expect(service.revokeVC('v1', 'req-1')).rejects.toMatchObject({
        code: ErrorCode.VC_NOT_FOUND,
      });
    });
    ...
    it('throws VC_ALREADY_REVOKED if already revoked', async () => {
      ...
      await expect(service.revokeVC('v1', 'req-1')).rejects.toMatchObject({
        code: ErrorCode.VC_ALREADY_REVOKED,
      });
    });
  });
  ...
  describe('renewVC branches', () => {
    it('throws VC_NOT_FOUND if old VC missing', async () => {
      repository.findByVcId.mockResolvedValue(null);
      await expect(service.renewVC('v1', {}, 'req-1')).rejects.toMatchObject({
        code: ErrorCode.VC_NOT_FOUND,
      });
    });
  ...
  describe('getVC/getVCStatus branches', () => {
    it('handles not found in getVC', async () => {
      repository.findByVcId.mockResolvedValue(null);
      await expect(service.getVC('v1', 'req-1')).rejects.toMatchObject({
        code: ErrorCode.VC_NOT_FOUND,
      });
    });
  ...
  describe('getStatusList branches', () => {
    it('throws STATUS_LIST_NOT_FOUND if missing', async () => {
      ...
      await expect(service.getStatusList('l1')).rejects.toMatchObject({
        code: ErrorCode.STATUS_LIST_NOT_FOUND,
      });
    });
  });
});
