// The Travel Planner chat surface.
//
// Deliberately server-rendered as one self-contained document: the demo has no
// bundler, and keeping it dependency-free means `docker compose up` is the only
// build step anyone needs.
//
// The right-hand rail is not decoration — it is the point of the demo. It shows
// the agent's own DID, who it is acting for, and exactly which scopes each
// Service Provider has granted, so a viewer can watch authorization appear as
// consent is given and see it reused on the second booking.

export function agentPageHtml(spOrigins: string[]): string {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<script>
const SP_ORIGINS = ${JSON.stringify(spOrigins)};
...
// Conversation state: whether the last agent turn was a question, and which
// search the gathering currently belongs to.
...
/** Renders a follow-up question with tappable answers. */
...
async function handle(text, opts) {
  ...
  try {
    const plan = await api('/api/plan', {
      // The answering flag tells the server this is a reply to its own
      // question, so a bare "2 travellers" continues the current search
    ...
    wait.remove();
    // The server may have fallen back to the scripted planner if the provider
    // was unavailable — reflect that in the badge rather than failing silently.
...
// Visit /?demo=1 and the whole journey plays itself at presentation pace, so a
// screen recording needs no operator. Consent popups auto-advance too (the
// demo flag is forwarded to the SP), which is what makes an unbroken take
// possible. Nothing here bypasses the credential flow — every step goes
// through the same routes a human click would.
...
async function runDemo() {
  ...
  // Flight results — pick the cheaper evening departure.
  await waitFor(() => optionButton('book-flight:').length >= 2);
  await pause(1900);
  optionButton('book-flight:')[1].click();

  // Consent hand-off: open the SP popup, which auto-advances in demo mode.
  await waitFor(() => document.querySelector('[data-action="consent"]'));
  await pause(1500);
  demoOpenConsent();

  // Booking confirms once the grant returns.
  await waitFor(() => document.querySelector('.receipt'), 45000);
  await pause(2400);

  // Hotel at the second, independent SP.
  ...
  // The payoff: same Airline SP again — no prompt, no new grant.
