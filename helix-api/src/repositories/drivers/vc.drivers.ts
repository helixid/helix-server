...
//
// Storage-driver implementations for VcRepository — see
// storage/driver-registry.ts for the pattern and
// repositories/drivers/did.drivers.ts for the original reference
// implementation. Every method here is a straight extraction of what used
// to be an if/else-per-backend branch inside VcRepository itself; behavior
// is unchanged.
...
/**
 * Everything VcRepository needs from a storage backend. Adding a new
 * backend means implementing this interface and wiring it into
 * createVcStorageDriver() below — VcRepository itself never changes.
 */
export interface VcStorageDriver {
  createVc(params: CreateVcParams): Promise<VCRecord>;
  ...
  revokeVc(vcId: string, listId: string, newEncodedList: string): Promise<VCRecord>;
  markAsRenewed(oldVcId: string, newVcId: string): Promise<VCRecord>;
}
...
function filterByType(records: VCRecord[], vcType?: string): VCRecord[] {
...
export class PrismaVcStorageDriver implements VcStorageDriver {
...
export class SqliteVcStorageDriver implements VcStorageDriver {
  ...
  async findActiveBySubjectDid(subjectDid: string, vcType?: string): Promise<VCRecord[]> {
    ...
    `);
    const records = rows.map((row) => fromSqliteVcRow(row)).filter((v): v is VCRecord => Boolean(v));
    return filterByType(records, vcType);
  }
...
export class InMemoryVcStorageDriver implements VcStorageDriver {
  ...
  private matchesStatus(record: VCRecord, status: ListVcFilters['status'], now = new Date()): boolean {
...
export function createVcStorageDriver(
  kind: StorageDriverKind,
  deps: StorageDriverDeps,
): VcStorageDriver {
  switch (kind) {
    case 'postgres':
      if (!deps.prisma) throw new Error("createVcStorageDriver('postgres') requires deps.prisma");
      return new PrismaVcStorageDriver(deps.prisma as PrismaClient);
    case 'sqlite':
      if (!deps.sqlite) throw new Error("createVcStorageDriver('sqlite') requires deps.sqlite");
      return new SqliteVcStorageDriver(deps.sqlite as SqliteStore);
    case 'memory':
      return new InMemoryVcStorageDriver();
    default: {
      const _exhaustive: never = kind;
      throw new UnsupportedStorageDriverError('VcRepository', _exhaustive);
    }
  }
}
